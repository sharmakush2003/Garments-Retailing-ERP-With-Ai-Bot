...
class SalesService {
    ...
    static async createSalesOrder(db, customerId, items, role = 'Customer') {
        try {
            const res = await axios.post(`${getBaseUrl()}/api/mock/create-sales-order`, { customerId, items, role });
            return res.data;
        } catch (err) {
            console.error("API create-sales-order call failed, falling back to local DB:", err.message);
            try {
    ...
    static async getDispatchTracking(db, orderId) {
        try {
            const res = await axios.get(`${getBaseUrl()}/api/mock/dispatch-tracking`, {
                params: { orderId }
            });
            return res.data;
        } catch (err) {
            console.error("API dispatch-tracking call failed, falling back to local DB:", err.message);
            const query = `
    ...
    static async getLastOrder(db, customerId) {
        try {
            const res = await axios.get(`${getBaseUrl()}/api/mock/last-order`, {
                params: { customerId }
            });
            return res.data;
        } catch (err) {
            console.error("API last-order call failed, falling back to local DB:", err.message);
            const orderQuery = `
